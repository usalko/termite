...
class MalletReader(LDAReader):
    ...
    def __init__(self, lda_db, modelPath):
        ...
        self.modelTopicWordWeights = '{}/{}'.format(
            self.modelPath, MalletReader.TOPIC_WORD_WEIGHTS_FILENAME)
        self.modelDocTopicMixtures = '{}/{}'.format(
            self.modelPath, MalletReader.DOC_TOPIC_MIXTURES_FILENAME)
    ...
    def ReadTopicWordWeights(self):
        ...
        termLookup = {term: termIndex for termIndex,
                      term in enumerate(self.termList)}
...
#					assert line == "#doc name topic proportion ..."
                    ...
                    docId = fields[1]
                    topicIndexes = [int(d) for n, d in enumerate(
                        fields[2:]) if n % 2 == 0 and d != '']
                    values = [float(d) for n, d in enumerate(
                        fields[2:]) if n % 2 == 1 and d != '']
        ...
        docLookup = {docId: docIndex for docIndex,
                     docId in enumerate(self.docList)}

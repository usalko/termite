...
class Corpus_DB():
    ...
    DEFAULT_OPTIONS = {
        # Tokenize a corpus into a bag-of-words language model
        'token_regex': r'\w{3,}',
    ...
    def __init__(self, path=None, isInit=False):
        self.isInit = isInit
        if path is not None:
            self.db = DAL(Corpus_DB.CONNECTION, lazy_tables=not self.isInit,
                          migrate_enabled=self.isInit, folder=path)
        else:
            self.db = DAL(
                Corpus_DB.CONNECTION, lazy_tables=not self.isInit, migrate_enabled=self.isInit)
...
################################################################################
    ...
    def GetOption(self, key):
        ...
        if keyValue and keyValue.values():
    ...
    def GetModels(self):
        rows = self.db(self.db.models).select(
            self.db.models.model_key, self.db.models.model_desc).as_list()
    ...
    def DefineMetadataTables(self):
        self.db.define_table('fields',
                             Field('field_index', 'integer',
                                   required=True, unique=True, default=-1),
...
################################################################################
    ...
    def ImportFromSpreadsheet(self, filename, id_key=None, content_key=None, is_csv=False):
        ...
        doc_content_keys = frozenset(
            [content_key] if content_key is not None else Corpus_DB.DOC_CONTENTS)
        field_indexes = []
        field_names = []
        field_types = []
        metadata = []
    ...
    def ExportToFile(self, filename):
        ...
        rows = self.db().select(self.db.corpus.doc_id, self.db.corpus.doc_content,
    ...
    def ExportToSpreadsheet(self, filename, id_key=None, content_key=None, is_csv=False):
        ...
        """
        field_names = [row.field_name for row in self.db().select(
            self.db.fields.field_name, orderby=self.db.fields.field_index)]
        ...
        def WriteTSV(rows):
            with open(filename, 'w') as f:
                f.write(u'{}\n'.format(u'\t'.join(
                    all_field_names)).encode('utf-8', 'ignore'))
                for row in rows:
                    doc_index = row.doc_index
                    doc_id = row.doc_id
                    doc_content = self.SanitizeText(
        ...
        rows = self.db().select(self.db.corpus.doc_index, self.db.corpus.doc_id,

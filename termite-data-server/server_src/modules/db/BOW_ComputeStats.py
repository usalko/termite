...
################################################################################
    ...
    def UnfoldStats(self, stats):
        ...
        for term, index in self.termLookup.items():
            if term in stats:
                data.append(
                    {'term_index': index, 'value': stats[term], 'rank': 0})
    ...
    def UnfoldCoStats(self, coStats):
        ...
        for first_term, stats in coStats.items():
            ...
            for second_term, value in stats.items():
                ...
                data.append({'first_term_index': first_term_index,
                            'second_term_index': second_term_index, 'value': value, 'rank': 0})
        data.sort(key=lambda x: -x['value'])
        for rank, d in enumerate(data):
            d['rank'] = rank+1
    ...
    def ComputeTermFreqs(self, corpus):
        ...
        def ComputeIdfs(corpus, docFreqs):
            N = len(corpus)
            idfs = {term: math.log(1.0 * N / docFreq)
                    for term, docFreq in docFreqs.items()}
            return idfs
    ...
    def ComputeVocabulary(self, termStats):
        ...
        keys = [term for term in termFreqs if termFreqs[term]
                >= minFreq and termDocFreqs[term] >= minDocFreq]
    ...
    def ComputeTermCoFreqs(self, corpus, termStats):
        ...
        def GetG2Stats(freq_all, freq_ab, freq_a, freq_b, a, b):
            ...
            if not (freq_ab >= 0):
                freq_ab = 0
                self.logger.warning(
                    'assertion failed: freq(%s, %s) >= 0', a, b)
            if not (freq_a >= freq_ab):
                freq_a = freq_ab
                self.logger.warning(
                    'assertion failed: freq(%s) >= freq(%s, %s)', a, a, b)
            if not (freq_b >= freq_ab):
                freq_b = freq_ab
                self.logger.warning(
                    'assertion failed: freq(%s) >= freq(%s, %s)', b, a, b)
            if not (freq_all >= freq_a and freq_all >= freq_b):
                freq_all = max(freq_a, freq_b)
                self.logger.warning(
                    'assertion failed: freq >= freq(%s) and freq >= freq(%s)', a, b)
                ...
                if firstToken in marginalProbs:
                    ...
                    for secondToken, jointProb in d.items():
                        if secondToken in marginalProbs:
                            ...
                            g2_stats[firstToken][secondToken] = GetG2Stats(
                                freq_all, freq_ab, freq_a, freq_b, firstToken, secondToken)
            return g2_stats

        self.logger.info(
            '    Computing term co-occurrences (%d docs)...', len(corpus))

        # Count co-occurrences
        termCoFreqs, allFreq, allCoFreq = ComputeJointFreqs(corpus, self.vocab)

        # Normalize to create joint probability distribution
        termCoProbs = NormalizeJointFreqs(
            termCoFreqs, 1.0 / allCoFreq if allCoFreq > 1.0 else 1.0)

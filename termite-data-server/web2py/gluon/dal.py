...
if PYTHON_VERSION[0] == 2:
    ...
    import copyreg
    long = int
    def hashlib_md5(s): return hashlib.md5(bytes(s, 'utf8'))
...
class BaseAdapter(ConnectionPool, metaclass=AdapterMeta):
    ...
    def create_table(self, table,
        ...
        if self.dbengine == 'mysql':
            if not hasattr(table, "_primarykey"):
                fields.append('PRIMARY KEY (%s)' %
                              (self.QUOTE_TEMPLATE % table._id.name))
        ...
        for rtablename in TFK:
            ...
            pkeys = [self.QUOTE_TEMPLATE %
                     pk for pk in db[rtablename]._primarykey]
        ...
        for key in keys:
            ...
            if not key in sql_fields_old:
                ...
                query = ['ALTER TABLE %s ADD %s %s;' % (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE % key_tmp, tt),
                         'UPDATE %s SET %s=%s;' %
                         (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE %
                          key_tmp, self.QUOTE_TEMPLATE % key),
                         drop_expr % (self.QUOTE_TEMPLATE %
                                      t, self.QUOTE_TEMPLATE % key),
                         'ALTER TABLE %s ADD %s %s;' %
                         (self.QUOTE_TEMPLATE %
                          t, self.QUOTE_TEMPLATE % key, tt),
                         'UPDATE %s SET %s=%s;' %
                         (self.QUOTE_TEMPLATE % t, self.QUOTE_TEMPLATE %
                          key, self.QUOTE_TEMPLATE % key_tmp),
    ...
    def insert(self, table, fields):
        ...
        if hasattr(table, '_primarykey'):
            mydict = dict([(k[0].name, k[1])
                          for k in fields if k[0].name in table._primarykey])
            if mydict != {}:
                return mydict
    ...
    def expand(self, expression, field_type=None, colnames=False):
        if isinstance(expression, Field):
            ...
            if not colnames:
                ...
                out = '%s.%s' % (table_rname, expression._rname or (
                    self.QUOTE_TEMPLATE % (expression.name)))
            else:
                out = '%s.%s' % (self.QUOTE_TEMPLATE % et._tablename,
                                 self.QUOTE_TEMPLATE % expression.name)
        ...
        if (limitby and not groupby and tablenames and orderby_on_limitby and not orderby):
            sql_o += ' ORDER BY %s' % ', '.join(
                [self.db[t].sqlsafe + '.' + self.db[t][x].sqlsafe_name for t in tablenames for x in (
                    hasattr(
                        self.db[t], '_primarykey') and self.db[t]._primarykey
                    or ['_id']
                )
                ]
            )
                ...
                if tmp:
                    ...
                    if ft == 'id' and not cacheable:
                        ...
                        if table._db._lazy_tables:
                            colset['__get_lazy_reference__'] = LazyReferenceGetter(
                                table, id)
    ...
    def CASE(self, query, t, f):
        def represent(x):
            types = {type(True): 'boolean', type(
                0): 'integer', type(1.0): 'double'}
            if x is None:
                return 'NULL'
...
class PostgreSQLAdapter(BaseAdapter):
    ...
    def try_json(self):
        ...
        if supports_json:
            ...
            LOGGER.debug(
                "Your database version does not support the JSON data type (using TEXT instead)")
...
class OracleAdapter(BaseAdapter):
    ...
    def constraint_name(self, tablename, fieldname):
        constraint_name = BaseAdapter.constraint_name(
            self, tablename, fieldname)
        if len(constraint_name) > 30:
            constraint_name = '%s_%s__constraint' % (
                tablename[:10], fieldname[:7])
    ...
    def execute(self, command, args=None):
        ...
        while True:
            ...
            if not m:
                break
            command = command[:m.start('clob')] + \
                str(i) + command[m.end('clob'):]
    ...
    def create_sequence_and_triggers(self, query, table, **args):
        ...
        self.execute(query)
        self.execute(
            'CREATE SEQUENCE %s START WITH 1 INCREMENT BY 1 NOMAXVALUE MINVALUE -1;' % sequence_name)
        self.execute("""
...
class MSSQL3Adapter(MSSQLAdapter):
    ...
    def select_limitby(self, sql_s, sql_f, sql_t, sql_w, sql_o, limitby):
        if limitby:
            ...
            sql_f_iproxy = ['%s AS %s' % (o, n) for (
                o, n) in zip(sql_f_inner, sql_f_outer)]
        ...
        if limitby:
            ...
            if lmin == 0:
                ...
                if not sql_o:
                    ...
                    sql_o += ' ORDER BY %s' % self.RANDOM()
                sql_o += ' OFFSET %i ROWS FETCH NEXT %i ROWS ONLY' % (
                    lmin, lmax - lmin)
        return 'SELECT %s %s FROM %s%s%s;' % \
            (sql_s, sql_f, sql_t, sql_w, sql_o)
...
class IngresAdapter(BaseAdapter):
    ...
    types = {
        ...
        'password': 'VARCHAR(%(length)s)',
        'blob': 'BLOB',
        # FIXME utf8 or nvarchar... or blob? what is this type?
        'upload': 'VARCHAR(%(length)s)',
    ...
    def create_sequence_and_triggers(self, query, table, **args):
        ...
        if hasattr(table, '_primarykey'):
            ...
            self.execute(query)
            self.execute('modify %s to btree unique on %s' %
                         (table._tablename, 'id'))
...
class DatabaseStoredFile:
    ...
    def __init__(self, db, filename, mode):
        if not db._adapter.dbengine in ('mysql', 'postgres', 'sqlite'):
            raise RuntimeError(
                "only MySQL/Postgres/SQLite can store metadata .table files in database for now")
        self.db = db
...
class NoSQLAdapter(BaseAdapter):
    ...
    def STARTSWITH(self, first, second=None): raise SyntaxError(
        "Not supported")
    ...
    def distributed_transaction_begin(
        self, key): raise SyntaxError("Not supported")
    ...
    def constraint_name(
        self, table, fieldname): raise SyntaxError("Not supported")
    ...
    def represent_exceptions(
        self, obj, fieldtype): raise SyntaxError("Not supported")

    def lastrowid(self, table): raise SyntaxError("Not supported")
    def rowslice(self, rows, minimum=0,
                 maximum=None): raise SyntaxError("Not supported")
...
class GoogleDatastoreAdapter(NoSQLAdapter):
    ...
    def create_table(self, table, migrate=True, fake_migrate=False, polymodel=None):
        ...
        for field in table:
            ...
            field_type = field.type
            if isinstance(field_type, SQLCustomType):
                ftype = self.types[field_type.native or field_type.type](
                    **attr)
                ...
                scale = int(scale)
                dec_cls = (
                    self.use_ndb and NDBDecimalProperty) or GAEDecimalProperty
            ...
            if orderby:
                ...
                for order in orders:
                    if self.use_ndb:
                        ...
                        _order = {'-id': -tableobj._key,
                                  'id': tableobj._key}.get(order)
                        if _order is None:
                            _order = make_order(order)
                        items = items.order(_order)
                    else:
                        order = {'-id': '-__key__',
                                 'id': '__key__'}.get(order, order)
    ...
    def bulk_insert(self, table, items):
        parsed_items = []
        for item in items:
            dfields = dict((f.name, self.represent(v, f.type))
                           for f, v in item)
    ...
    *At the application side it is measured as the length of the RFC822
    ...
    def select(self, query, fields, attributes):
        ...
        if use_common_filters(query):
            query = self.common_filter(
                query, [self.get_query_mailbox(query), ])
        ...
        if isinstance(query, Query):
            ...
            if mailbox is None:
                ...
                search_result = self.connection.uid(
                ...
                if search_result[0] == "OK":
                    ...
                    if limitby is not None:
                        ...
                        messages_set = messages_set[int(
                            limitby[0]):int(limitby[1])]
        ...
        for fr in fetch_results:
            ...
            if "%s.created" % tablename in colnames:
                item_dict["%s.created" %
                          tablename] = self.convert_date(message["Date"])
    ...
    def insert(self, table, fields):
        ...
        if len(d) > 0:
            ...
            result, data = self.connection.append(
                mailbox, flags, struct_time, message)
            if result == "OK":
    ...
    def update(self, tablename, query, fields):
        ...
        if query:
            ...
            for number in store_list:
                result = None
                if len(mark) > 0:
                    commands.append((number, "+FLAGS", "(%s)" %
                                    " ".join(mark)))
                if len(unmark) > 0:
                    commands.append((number, "-FLAGS", "(%s)" %
                                    " ".join(unmark)))
    ...
    def count(self, query, distinct=None):
        ...
        if query and tablename is not None:
            ...
            string_query = "(%s)" % query
            result, data = self.connection.search(None, string_query)
            store_list = [item.strip()
                          for item in data[0].split() if item.strip().isdigit()]
    ...
    def CONTAINS(self, first, second, case_sensitive=False):
        ...
        if name in ("FROM", "TO", "SUBJECT", "TEXT"):
            ...
            if first.name in ("cc", "bcc"):
                result = "%s \"%s\"" % (
                    first.name.upper(), self.expand(second))
    ...
    def GT(self, first, second):
        ...
        if name == "MESSAGE":
            ...
            result = "SINCE %s" % self.convert_date(
                second, add=datetime.timedelta(1))
        elif name == "SIZE":
    ...
    def LE(self, first, second):
        ...
        if name == "MESSAGE":
            ...
            result = "BEFORE %s" % self.convert_date(
                second, add=datetime.timedelta(1))
        else:
            raise Exception("Operation not supported")
        return result
...
def sqlhtml_validators(field):
    ...
    if field_type in (('string', 'text', 'password')):
        ...
        requires.append(validators.IS_EMPTY_OR(
            validators.IS_JSON(native_json=field.db._adapter.native_json)))
        ...
        def list_ref_repr(ids, row=None, r=referenced, f=ff):
            ...
            if isinstance(db._adapter, GoogleDatastoreAdapter):
                ...
                refs = functools.reduce(lambda a, b: a & b, [
                                        count(ids[i:i+30]) for i in rx])
            else:
        ...
        def repr_list(values, row=None): return', '.join(str(v)
                                                         for v in (values or []))
...
class Row(object):
    ...
    def __setitem__(self, key, value): return setattr(self, str(key), value)

    __delitem__ = object.__delattr__

    def __copy__(self): return Row(self)
    ...
    update = lambda self, * \
        args, **kwargs:  self.__dict__.update(*args, **kwargs)
    ...
    def iteritems(self): return self.__dict__.items()
    ...
    def __init__(self, uri=DEFAULT_URI,
        ...
        * '<adaptername>_nonreserved' Checks against the specific adapters
        ...
        if not decode_credentials:
            def credential_decoder(cred): return cred
        else:
            def credential_decoder(cred): return urllib.parse.unquote(cred)
        self._folder = folder
        if folder:
                ...
                for uri in uris:
                    try:
                        ...
                        if not self._dbname in ADAPTERS:
                            raise SyntaxError(
                                "Error in URI '%s' or database not supported" % self._dbname)
            ...
            for tag in tags:
                if re1.match(tag):
                    ...
                    if '.' in ref and otable:
                        ...
                        if db[table][field].type.startswith('reference '):
                            refs = [x.name for x in db[otable]
                                    if x.type == db[table][field].type]
                        else:
                            refs = [
                                x.name for x in db[table]._referenced_by if x.tablename == otable]
                        if refs:
                            selfld = refs[0]
                        if nested_select:
                            try:
                                dbset = db(db[table][field].belongs(
                                    dbset._select(db[otable][selfld])))
                            except ValueError:
                                return Row({'status': 400, 'pattern': pattern,
                                            'error': 'invalid path', 'response': None})
                        else:
                            items = [item.id for item in dbset.select(
                                db[otable][selfld])]
                    ...
                    items = dbset.select(
                        db[table][field], distinct=distinct, limitby=limits)
                    if items:
                ...
                if i == len(tags) and table:
                    if hasattr(db[table], '_id'):
                        ofields = vars.get(
                            'order', db[table]._id.name).split('|')
                    else:
                        ofields = vars.get(
                            'order', db[table]._primarykey[0]).split('|')
                    try:
                    ...
                    if exposedfields:
                        fields = [field for field in db[table] if str(field).split(
                            '.')[-1] in exposedfields and field.readable]
                    else:
                        fields = [field for field in db[table]
                                  if field.readable]
                    count = dbset.count()
                    try:
                        offset = int(vars.get('offset', None) or 0)
...
class Table(object):
    ...
    def _enable_record_versioning(self,
        ...
        if is_active and is_active in fieldnames:
            ...
            def newquery(query, t=self, name=self._tablename): return \
    ...
    def __call__(self, key=DEFAULT, **kwargs):
        ...
        if not key is DEFAULT:
            ...
            query = functools.reduce(lambda a, b: a & b, [
                                     self[k] == v for k, v in kwargs.items()])
    ...
    def bulk_insert(self, items):
        ...
        ret and [[f(item, ret[k]) for k, item in enumerate(items)]
            ...
            if not colnames:
                ...
                if not (id_map or cid is None or id_offset is None or unique_idx):
                    ...
                    while curr_id < csv_id+id_offset[self._tablename]:
                        self._db(self._db[self][colnames[cid]]
                                 == curr_id).delete()
                        curr_id = self.insert(**dict(items))
...
class Field(Expression):
    ...
    def __init__(
        ...
        self.length = length if not length is None else DEFAULTLENGTH.get(
            self.type, 512)
    ...
    def store(self, file, filename=None, path=None):
        ...
        newfilename = newfilename[:(
            self.length - 1 - len(extension))] + '.' + extension
...
class Rows(object):
    ...
    def __getitem__(self, i):
        row = self.records[i]
        keys = row.keys()
        if self.compact and len(keys) == 1 and next(iter(keys)) != '_extra':
    ...
    def as_dict(self,
        ...
        if f and isinstance(key, (str, bytes)):
            ...
            if (not "." in key) and multi:
                ...
                def key(r): return key_generator.next()

        rows = self.as_list(compact, storage_to_dict,
                            datetime_to_str, custom_types)
